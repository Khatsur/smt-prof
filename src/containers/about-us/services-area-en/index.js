...
import ServiceBox from '../../../components/box-image/layout-six-en'
...
const Services = ({serviceBoxStyle, linkStyle, headingStyle}) => {
    const featuredDataQuery = useStaticQuery(graphql `
        query AboutServicesEnQueryData {
    ...
    return (
        <ServicesWrapper> 
            <Container>
                <Row>
                    {services && services.map(service => (
                        <Col lg={4} md={6} key={service.node.id} {...serviceBoxStyle}>
                            <ServiceBox
                                title={service.node.titleen}
                                desc={service.node.excerpten}
                                imageSrc={service.node.icon.img.childImageSharp}
                                path={`${service.node.pathen}`}
                            />
                        </Col>
                    ))}
                </Row>
                <Row>
                    <Col lg={12}>
                        <Heading {...headingStyle}>Complete automation of production. <Anchor {...linkStyle} path="/en/lines">Take the challenge!</Anchor></Heading>
                    </Col>
                </Row>
            </Container>
        </ServicesWrapper>
